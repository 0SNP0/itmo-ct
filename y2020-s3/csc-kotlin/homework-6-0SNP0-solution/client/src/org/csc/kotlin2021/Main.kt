...
fun checkHostAndPort(host: String, port: Int) {
    host.split(".").forEach {
        """[a-z0-9-]+""".toRegex().matchEntire(it) ?: throw IllegalArgumentException("Illegal host name '$host'")
    }
    if (port !in VALID_PORT_NUMS) throw IllegalArgumentException("Illegal port value '$port'")
}
val VALID_PORT_NUMS = 0..65535 // 2^16 - 1
