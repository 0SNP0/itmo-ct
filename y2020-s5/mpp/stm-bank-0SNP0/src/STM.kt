...
class TxVar<T>(initial: T) {
    ...
    fun openIn(tx: Transaction, update: (T) -> T): T {
        while (true) {
            ...
            val curValue = if (tx === curLoc.owner) curLoc.newValue else when (curLoc.owner.status) {
                TxStatus.ACTIVE -> curLoc.owner.abort().let { null }
                TxStatus.COMMITTED -> curLoc.newValue
                TxStatus.ABORTED -> curLoc.oldValue
            } ?: continue
            val updValue = update(curValue)
            if (loc.compareAndSet(curLoc, Loc(curValue, updValue, tx))) return updValue
        }
    }
}
